...
#define MMC_CD		(!(PING & 0x01))	/* Card detected.   yes:true, no:false, default:true */
#define MMC_WP		(PING & 0x02)		/* Write protected. yes:true, no:false, default:false */
...
)
{
	...
	if (cmd != CMD12) {
		deselect();
		if (!select()) return 0xFF; // ENTRA ACA Y DEVUELVE 255
	}
